...
function requireBackend(): never {
  throw new AppError(
    "CONFIGURATION",
    "Configura Supabase para consultar datos de personas.",
  );
}
...
export async function createPerson(input: PersonPayload): Promise<Person> {
  ...
  if (isLocalFallback()) {
    ...
    if (persons.some((person) => person.email === payload.email))
      throw new AppError("DUPLICATE", "El correo ya está registrado.");
    const person: Person = {
      ...payload,
...
export async function updatePerson(
  ...
  if (isLocalFallback()) {
    ...
    if (index < 0)
      throw new AppError("NOT_FOUND", "No se encontró la persona.");
    if (
      persons.some(
        (person) => person.id !== id && person.email === payload.email,
      )
    )
      throw new AppError("DUPLICATE", "El correo ya está registrado.");
