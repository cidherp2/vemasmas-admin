...
export function PersonsPage(): React.ReactElement {
  ...
  const openEdit = (person: Person): void => {
    resetFormState();
    ...
    <div className="space-y-7">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div>
          ...
          <div className="flex items-center gap-3">
            ...
            <Badge variant="default">{persons?.length ?? 0}</Badge>
          </div>
          <p className="mt-2 max-w-xl text-sm text-muted-foreground">
            Una vista viva de las personas que hacen avanzar tu organización.
          </p>
        </div>
      ...
      <Card>
        <CardHeader className="gap-4 border-b sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Busca por nombre o correo y filtra por estado.
            </p>
          </div>
        ...
        {!isLoading && !isError && filteredPersons?.length === 0 ? (
          <CardContent className="flex min-h-64 flex-col items-center justify-center px-5 text-center">
            ...
            <h2 className="font-display text-lg font-semibold">
              {hasFilters ? "No hay coincidencias" : "Tu directorio está vacío"}
            </h2>
            <p className="mt-1 max-w-sm text-sm text-muted-foreground">
              {hasFilters
                ? "Prueba con otra búsqueda o limpia los filtros activos."
                : "Añade a la primera persona para empezar a construir tu vista de equipo."}
            </p>
